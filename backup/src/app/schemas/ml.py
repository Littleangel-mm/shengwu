...
class MLRunCreate(BaseModel):
    ...
    def validate_algorithms(self) -> "MLRunCreate":
        regression = {"ridge", "pls", "svr", "random_forest", "gradient_boosting", "xgboost", "mlp"}
        classification = {"logistic", "svc", "random_forest", "gradient_boosting", "xgboost", "mlp"}
        allowed = regression if self.task_type == "regression" else classification
